...
const PetView: FC<{ tokenID: number; name: string }> = ({ tokenID, name }) => {
  ...
  const contract = useMemo<Wagmiabi>(
    () =>
      WAGMIpet.connect(
  ...
  const { data: stats, mutate: mutateStats } = useSWR(
    ...
    () =>
      contract
        .getStats(tokenID)
        .then((stats) => stats.map((stat) => stat.toNumber())),
    { revalidateOnFocus: false, refreshInterval: 1000 * 60 },
  );

  const interactWithPet = async (
    getTransaction: Promise<PopulatedTransaction>,
  ) => {
    ...
    const tx = await provider
      ...
      .catch((error) => {
        ...
        )?.error?.message?.split('execution reverted: ')?.[1];
  ...
  return (
    <div className="w-full max-w-screen-md mx-4 md:mx-0 space-y-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button
          onClick={() =>
            interactWithPet(contract.populateTransaction.clean(tokenID))
          }
          className="text-3xl py-2 px-6 border-4 border-current text-black dark:text-white hover:text-gray-500 dark:hover:text-gray-400 disabled:text-gray-400 dark:disabled:text-gray-500"
          disabled={!biconomy}
        >
          Clean
        </button>
        <button
          onClick={() =>
            interactWithPet(contract.populateTransaction.feed(tokenID))
          }
          className="text-3xl py-2 px-6 border-4 border-current text-black dark:text-white hover:text-gray-500 dark:hover:text-gray-400 disabled:text-gray-400 dark:disabled:text-gray-500"
          disabled={!biconomy}
        >
        ...
        <button
          onClick={() =>
            interactWithPet(contract.populateTransaction.play(tokenID))
          }
          className="text-3xl py-2 px-6 border-4 border-current text-black dark:text-white hover:text-gray-500 dark:hover:text-gray-400 disabled:text-gray-400 dark:disabled:text-gray-500"
          disabled={!biconomy}
        >
        ...
        <button
          onClick={() =>
            interactWithPet(contract.populateTransaction.sleep(tokenID))
          }
          className="text-3xl py-2 px-6 border-4 border-current text-black dark:text-white hover:text-gray-500 dark:hover:text-gray-400 disabled:text-gray-400 dark:disabled:text-gray-500"
          disabled={!biconomy}
        >
